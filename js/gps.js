...
const parkTimeEl = document.getElementById("parkTime");  // v83_test9: display total parking time
  ...
  // Sharp Turn detection state (v84_1128, v2.2 single-S per turn)
  const sharpState = {
    window: [],   // sliding window of up to 4 points
    inTurn: false,
    cooldown: 0   // small cool-down (in samples) after離開彎，避免立刻再觸發
  };
...
function filterPipeline(lat, lon, speedKmh, courseDeg, sats, hdop, dt){
  ...
  function finalizeParkCluster(){
    ...
    if (span >= 20) {
      // v83_test9: accumulate total parking time (seconds) and create parking event
      totalParkTimeSec += span;
      addParkingEvent(parkCluster.anchorLat, parkCluster.anchorLon);
      updateParkSummary();
    }
  }

  function addParkingEvent(lat, lon){
    const evt = { lat, lon };
    parkEvents.push(evt);
    parkCount++;
  ...
  function updateTracksAndMap(lat, lon, speedKmh, courseDeg, sats, hdop, fixOk){
    ...
    // Sharp Turn v2.2 detection（單一 S＋同側轉彎，一個彎只畫一次）
    if (fixOk && !Number.isNaN(courseDeg) && !Number.isNaN(speedKmh) && hdop != null) {
      if (!sharpState.window) sharpState.window = [];
      // 簡單的 cool-down，每次進來遞減
      if (sharpState.cooldown > 0) {
        sharpState.cooldown--;
      }
      ...
      if (sharpState.window.length === 4) {
        ...
        if (Math.abs(d12) >= 15 && g1.hdop != null && g1.hdop < 2.5) {
          ...
          const sameSide = (s1 !== 0 && s1 === s2 && s1 === s3); // 一路同側轉彎
          const accHeading = Math.abs(d12) + Math.abs(d13) + Math.abs(d14);
          const avgSpeed = (g2.speedKmh + g3.speedKmh + g4.speedKmh) / 3;

          // 事件觸發：僅在尚未進入彎、且無 cool-down 時才畫 S
          if (sameSide && accHeading >= sharpAngleDeg && avgSpeed >= sharpSpeedKmh) {
            if (!sharpState.inTurn && sharpState.cooldown <= 0) {
              if (sharpLayerGroup) {
                const sPos = L.latLng(g3.lat, g3.lon);
              ...
              if (sharpPtsEl) sharpPtsEl.textContent = sharpCount;

              // 標記已在這個彎畫過 S
              sharpState.inTurn = true;
            }
          }

          // 離開彎判斷：不再同側、或轉角回到接近直行，就視為離開本次彎
          if (sharpState.inTurn) {
            if (!sameSide || Math.abs(d12) < 5) {
              sharpState.inTurn = false;
              sharpState.cooldown = 3; // 短暫 cool-down，避免剛拉直又立刻觸發下一個彎
            }
          }
        }
      }
...
// ==============================
  ...
  function showRuleModal(type){
    if(type==="raw"){
      ...
      ruleBody.innerHTML = `
        <p><b>用途：</b>標示車輛在同一地點附近停留超過一定時間的停車點（P icon）。</p>
        <ul>
          <li><b>速度：</b>當速度 ≤ 3 km/h 視為停車候選。</li>
          <li><b>距離：</b>第一個候選點為 anchor，之後距離 anchor ≤ 10 m 視為同一停車段。</li>
          <li><b>時間：</b>停在 anchor 附近累積 ≥ 20 秒才成立停車事件。</li>
          <li><b>時間軸：</b>優先使用 UTC 秒數，沒有 UTC 才改用實際 dt。</li>
          <li><b>顯示：</b>每一段停車只在 anchor 打一個 P icon，點數顯示在「停車」統計欄。</li>
        </ul>
      `;
} else if(type==="sharp"){
      ruleTitle.textContent="🔴 急轉向 Sharp Turn 規則";
      ruleBody.innerHTML = `
        <p><b>用途：</b>偵測「急轉向」行為，作為 UBI 與駕駛習慣評估指標之一。</p>
        <ul>
          <li><b>視窗：</b>使用連續 4 點 g1, g2, g3, g4 作為偵測視窗。</li>
          <li><b>g1 鎖定：</b>|heading(g2) − heading(g1)| ≥ 15°，且 g1 的 HDOP &lt; 2.5。</li>
          <li><b>同側條件：</b>g2, g3, g4 相對 g1 的角度差 (d12, d13, d14) 符號相同且非 0，代表一路同側轉彎（純左彎或純右彎）。</li>
          <li><b>角度門檻：</b>|d12| + |d13| + |d14| ≥「角度」下拉選單的設定值。</li>
          <li><b>速度門檻：</b>(v2 + v3 + v4) / 3 ≥「速度」下拉選單的設定值。</li>
          <li><b>事件行為（v2.2）：</b>第一個符合條件的視窗，在 g3 畫 1 顆紅色 S，之後進入 inTurn 狀態，直到離開彎並經過短暫 cool-down 才允許下一個急轉向事件。</li>
          <li><b>顯示統計：</b>右上「急轉向 (N 點)」中的 N 為本次軌跡偵測到的 S icon 數量。</li>
        </ul>
      `;
} else if(type==="ver"){
      ...
      ruleBody.innerHTML = `
        <p><b>v84_1128</b></p>
        <ul>
          <li>新增急轉向偵測（Sharp Turn v2.2）：一個彎只畫一次 S，並加入 inTurn / cool-down 狀態控制。</li>
          <li>急轉向統計與說明：顯示「急轉向 (N 點)」，並新增規則說明條目，方便 UBI 解釋。</li>
        </ul>
        <p><b>v77_1126</b></p>
        <ul>
  ...
  // 若頁面上已有 RN 專用的 verModalBackdrop（例如 GPS 頁），則避免重複綁定 verInfo 到 ruleModal
  const verModalBackdrop = document.getElementById("verModalBackdrop");
  if(verInfo && !verModalBackdrop){
    verInfo.addEventListener("click", e=>{
      e.stopPropagation();
      showRuleModal("ver");
    });
  }
