        ...
        if (sharpState.window.length === 4) {
          ...
          if (Math.abs(d12) >= 15 && g1.hdop != null && g1.hdop < 2.5) {
            ...
            if (s1 !== 0 && s1 === s2 && s1 === s3) {
              ...
              if (accHeading >= sharpAngleDeg && avgSpeed >= sharpSpeedKmh) {
                // Sharp turn event triggered at g3（v2.3：一律使用全域 10 秒冷卻）
                if (sharpLayerGroup) {
    ...
    if(type==="raw"){
      ...
      ruleBody.innerHTML = `
        ...
        <ul>
          ...
          <li><b>事件行為（v2.3）：</b>第一個符合條件的視窗，在 g3 畫 1 顆紅色 S icon，事件成立後啟動 10 秒全域冷卻計時器；冷卻期間即使有新的視窗符合條件也不再產生新的 S，只有冷卻結束後才允許下一個急轉向事件。</li>
